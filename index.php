	...
	switch ($action) {
		case 'attendance-add':
			if (isset($_POST['add'])) {
				...
				$attendance_date = date("d-m-Y", $attendance_timestamp);

				if (!empty($_POST['student_id'])) {
					$attendance->deleteAttendanceByDate(
								$attendance_date);
			...
			case 'attendance-edit':
				...
				case 'student-add':
					if (isset($_POST['add'])) {
						...
						if (empty($insertId)) {
							$response = array("message" => "Problem in adding new record", 
								"type" => "error");
						} else{
